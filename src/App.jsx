...
// Colour palette — cool tones only, distinct from UI red/yellow/green/orange
const GROUP_COLORS = [
...
// ── SelectGrid with RT colour group support ───────────────────────────────────
  ...
  return (
    <div className="grid gap-1" style={{gridTemplateColumns:`repeat(${Math.min(count,10)},minmax(0,1fr))`}}>
      {Array.from({length:count},(_,i)=>i+1).map(n=>{
        ...
        let cls="aspect-square rounded text-xs font-bold flex items-center justify-center transition-all";
        let sty={};
        ...
        return (
          <button key={n} disabled={isOut||isUnavail} onClick={()=>onToggle(n)}
            className={cls} style={sty} title={isUnavail?"Temporarily unavailable":grp?grp.label:undefined}>
            {isUnavail&&!isOut?"✕":n}
          </button>
        );
      })}
    ...
    <div className="grid gap-1" style={{gridTemplateColumns:`repeat(${Math.min(nums.length,10)},minmax(0,1fr))`}}>
      {sortNums(nums).map(n=>{ const isRet=returning.includes(n); return (
        <button key={n} onClick={()=>onToggle(n)}
          className={`aspect-square rounded text-xs font-bold flex items-center justify-center transition-all ${isRet?"bg-green-100 text-green-700 hover:bg-red-50":"bg-red-100 text-red-600 ring-1 ring-red-300"}`}>
          {n}
        </button>
      ); })}
    </div>
  ...
  return (
    <div className="grid gap-1" style={{gridTemplateColumns:`repeat(${Math.min(count,10)},minmax(0,1fr))`}}>
      {Array.from({length:count},(_,i)=>i+1).map(n=>{ const isU=unavailList.includes(n); return (
        <button key={n} onClick={()=>onToggle(n)}
          className={`aspect-square rounded text-xs font-bold flex items-center justify-center transition-all ${isU?"bg-amber-400 text-white":"bg-slate-100 text-slate-600 hover:bg-amber-100"}`}>
          {n}
        </button>
      ); })}
    </div>
...
// Grid for assigning RTs to a colour group in settings
  ...
  return (
    <div className="grid gap-1" style={{gridTemplateColumns:`repeat(${Math.min(count,10)},minmax(0,1fr))`}}>
      {Array.from({length:count},(_,i)=>i+1).map(n=>{ const inG=groupRts.includes(n),inO=otherRts.includes(n); return (
        <button key={n} disabled={inO} onClick={()=>onToggle(n)}
          ...
          title={inO?"In another group":undefined}>
          {n}
        </button>
      ); })}
    </div>
              ...
              <div className="mb-4">
                ...
                <div className="grid grid-cols-10 gap-1">
                  {Array.from({length:settings.rtCount},(_,i)=>i+1).map(n=>{
                    ...
                    const grp=getRtGroup(n,settings.rtGroups);
                    const gc=grp?GC[grp.color]:null;
                    let cls="aspect-square rounded text-xs font-bold flex items-center justify-center transition";
                    let sty={};
                    ...
                    return (
                      <button key={n}
                        onClick={()=>(isOut||isUnavail)&&setTooltip(t=>t?.num===n&&t?.type==="rt"?null:{type:"rt",num:n,owner:isOut?owners[`rt-${n}`]:null})}
                        className={cls} style={sty} title={grp&&!isOut&&!isUnavail?grp.label:undefined}>
                        {isUnavail&&!isOut?"✕":n}
                      </button>
                    );
                  })}
                    ...
                    <div className="text-xs flex flex-wrap gap-1">
                      {sortNums([...outSet]).map(n=>(
                        <span key={`o${n}`} className="inline-block bg-slate-200 rounded px-1.5 py-0.5 cursor-pointer hover:bg-slate-300"
                          onClick={()=>setTooltip(t=>t?.num===n&&t?.type===type?null:{type,num:n,owner:owners[`${type}-${n}`]})}>#{n}</span>
                      ))}
                      {sortNums([...unavSet]).filter(n=>!outSet.has(n)).map(n=>(
                        <span key={`u${n}`} className="inline-block bg-amber-100 text-amber-600 rounded px-1.5 py-0.5 cursor-pointer"
                          onClick={()=>setTooltip(t=>t?.num===n&&t?.type===type?null:{type,num:n,owner:null})}>#{n}✕</span>
                      ))}
                    </div>
                  )}
                </div>
          ...
          <div>
            ...
            {staff.length===0?(
              <div className="text-center py-12 text-slate-400 text-sm">
                <p>No staff added yet.</p><button onClick={()=>goTo(PAGES.STAFF)} className="mt-2 text-blue-600 underline">Go to Staff tab</button>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm p-5 flex flex-col gap-4">
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={()=>{setPickerSearch("");goTo(PAGES.PICKER);}}
                    className="shrink-0 text-sm bg-slate-100 hover:bg-slate-200 border border-slate-300 text-slate-700 font-medium px-3 py-2 rounded-xl transition">👥 Staff</button>
                </div>
              </div>
              <div>
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-600 mb-2">
                  Channel <span className="font-normal text-slate-400">(optional)</span>
                </label>
                <div className="flex gap-1.5">
                  {[1,2,3,4,5,6,7,8].map(ch=>(
                    <button key={ch} onClick={()=>setCf(f=>({...f,channel:f.channel===ch?null:ch}))}
                      className={`w-9 h-9 rounded-xl text-sm font-bold flex items-center justify-center transition
                        ${cf.channel===ch?"bg-blue-600 text-white":"bg-slate-100 text-slate-600 hover:bg-blue-100"}`}>
                      {ch}
                    </button>
                  ))}
                </div>
                      ...
                      {members.map(s=>(
                        <div key={s.id} className="bg-white rounded-xl border border-slate-200 px-4 py-3 flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            {editingId===s.id?(
                              <select className="border border-slate-300 rounded-lg px-2 py-1 text-xs focus:outline-none bg-white" value={editRole} onChange={e=>setEditRole(e.target.value)}>
                                {ROLES.map(r=><option key={r}>{r}</option>)}
                              </select>
                            ):(
            ...
            {!settingsUnlocked?(
              <div className="bg-white rounded-2xl shadow-sm p-6 max-w-sm mx-auto">
                ...
                <input type="password" className="w-full border border-slate-300 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 mb-3"
                  placeholder="Password" value={settingsPw} onChange={e=>setSettingsPw(e.target.value)} onKeyDown={e=>e.key==="Enter"&&checkPw()}/>
              ...
              <div className="flex flex-col gap-5">
                ...
                <div className="bg-white rounded-2xl shadow-sm p-4">
                  ...
                  <p className="text-xs text-slate-400 mb-4">Assign colours to groups of RTs for visual identification on the dashboard and checkout.</p>

                  {(settings.rtGroups||[]).map(group=>{
                    const gc=GC[group.color]||GC.purple;
                    ...
                    return (
                      <div key={group.id} className="mb-3 rounded-xl border border-slate-200 overflow-hidden">
                        <div className="flex items-center justify-between px-3 py-2.5" style={{backgroundColor:gc.light}}>
                          ...
                          <div className="flex gap-2">
                            <button onClick={()=>{setEditingGroupId(group.id);setEditGroup({...group,rts:[...(group.rts||[])]});}}
                              className="text-xs px-2.5 py-1 rounded-lg bg-white bg-opacity-60 text-slate-600 hover:bg-opacity-100 transition">Edit</button>
                        ...
                        {!isEditing&&(group.rts||[]).length>0&&(
                          <div className="px-3 py-2 text-xs text-slate-500 border-t border-slate-100">RTs: {sortNums(group.rts).join(", ")}</div>
                        )}
                        {isEditing&&editGroup&&(
                          <div className="p-3 bg-slate-50 border-t border-slate-200">
                            <input className="w-full border border-slate-300 rounded-xl px-3 py-2 text-sm mb-3 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                              value={editGroup.label} onChange={e=>setEditGroup(f=>({...f,label:e.target.value}))} placeholder="Group label"/>
                            <div className="flex gap-2 mb-3">
                              {GROUP_COLORS.map(c=>(
                                <button key={c.id} onClick={()=>setEditGroup(f=>({...f,color:c.id}))}
                                  className={`w-7 h-7 rounded-full transition ${editGroup.color===c.id?"ring-2 ring-offset-1 ring-slate-600":""}`}
                                  style={{backgroundColor:c.dark}}/>
                              ))}
                            </div>
                            <p className="text-xs text-slate-500 mb-2">Tap to assign RTs:</p>
                            <GroupRtGrid count={settings.rtCount} groupRts={editGroup.rts} otherRts={otherRts} color={editGroup.color}
                              onToggle={n=>setEditGroup(f=>({...f,rts:f.rts.includes(n)?f.rts.filter(x=>x!==n):[...f.rts,n]}))}/>
                  ...
                  {showAddGroup?(
                    <div className="p-3 border-2 border-blue-200 bg-blue-50 rounded-xl">
                      <input className="w-full border border-slate-300 rounded-xl px-3 py-2 text-sm mb-3 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                        placeholder="Group label (e.g. Old RTs, New RTs)" value={newGroup.label}
                        onChange={e=>setNewGroup(f=>({...f,label:e.target.value}))}/>
                      <div className="flex gap-2 mb-3">
                        {GROUP_COLORS.map(c=>(
                          <button key={c.id} onClick={()=>setNewGroup(f=>({...f,color:c.id}))}
                            className={`w-7 h-7 rounded-full transition ${newGroup.color===c.id?"ring-2 ring-offset-1 ring-slate-600":""}`}
                            style={{backgroundColor:c.dark}}/>
                        ))}
                      </div>
                      <p className="text-xs text-slate-500 mb-2">Tap to assign RTs:</p>
                      <GroupRtGrid count={settings.rtCount}
                        groupRts={newGroup.rts}
                        otherRts={(settings.rtGroups||[]).flatMap(g=>g.rts||[])}
                        color={newGroup.color}
                        onToggle={n=>setNewGroup(f=>({...f,rts:f.rts.includes(n)?f.rts.filter(x=>x!==n):[...f.rts,n]}))}/>
                      <div className="flex gap-2 mt-3">
